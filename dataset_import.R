    ...
    tabsetPanel(
      ...
      tabPanel(
        ...
        selectInput(
          ...
          c("CTRPv2","GDSC1", "GDSC2","PRISM Repurposing")
        ),
...
datasetServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    fileInfo <- reactiveValues(dataset = preprovided_dataset[["CTRPv2"]], extraCol = c("seCol", "subCol"), type = "provided", efficacyMetric = "Viability", isLowerEfficacy = T, cellLinesAndSubgroups = distinct(preprovided_dataset[["CTRPv2"]][, c("Cell_Line","Cell_Line_Subgroup")]), name = "CTRPv2", doseUnit = "microMolar")
